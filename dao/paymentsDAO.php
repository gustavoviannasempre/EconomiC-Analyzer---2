...
class paymentsDAO
{
    ...
    public function atualizar($payment){
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_payment, tb_city_id_city, tb_functions_id_function, tb_subfunctions_id_subfunction, tb_program_id_program, tb_action_id_action, tb_beneficiaries_id_beneficiaries, tb_source_id_source, tb_files_id_file, int_month, int_year, db_value FROM tb_payments WHERE id_payment = :id");
            $statement->bindValue(":id", $payment->getIdPayment());
            if ($statement->execute()) {
    ...
    public function tabelapaginada() {
        ...
        $sql = "SELECT P.id_payment, B.str_name_person as tb_beneficiaries, P.tb_city_id_city, P.tb_functions_id_function, P.tb_subfunctions_id_subfunction, P.tb_program_id_program,
                                                 P.tb_action_id_action, P.tb_source_id_source, P.tb_files_id_file, P.int_month, P.int_year, P.db_value
                                                 FROM tb_payments P INNER JOIN tb_beneficiaries B ON B.id_beneficiaries = P.tb_beneficiaries_id_beneficiaries LIMIT {$linha_inicial}, " . QTDE_REGISTROS;


        //$sql = "SELECT id_payment, tb_city_id_city, tb_functions_id_function, tb_subfunctions_id_subfunction, tb_program_id_program, tb_action_id_action, tb_beneficiaries_id_beneficiaries, tb_source_id_source, tb_files_id_file, db_value FROM tb_payments LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
     ...
     <tbody>";
        ...
        <td style='text-align: center'>$paym->tb_beneficiaries</td>
